...
def BPR_train_original(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    timer.zero()
    wandb.log({"epoch": epoch, "Train/loss": aver_loss})
    ...
    with torch.no_grad():
        ...
        if world.tensorboard:
            ...
            wandb.log(
                {
                    "epoch": epoch,
                    f"Test/Recall@{world.topks}": results["recall"],
                    f"Test/Precision@{world.topks}": results["precision"],
                    f"Test/NDCG@{world.topks}": results["ndcg"],
                }
            )
