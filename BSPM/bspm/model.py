...
class BSPM(object):
    def __init__(self, adj_mat, config: dict):
        ...
        print(
            f"IDL: {self.idl_solver}, BLR: {self.blur_solver}, SHR: {self.sharpen_solver}"
        )
    ...
    def getUsersRating(self, batch_users, ds_name):
        ...
        with torch.no_grad():
            if ds_name != "amazon-book":
                idl_out = odeint(
                    func=self.IDLFunction,
                    y0=torch.Tensor(batch_test),
                    t=self.idl_times,
                    method=self.idl_solver,
                )

            blurred_out = odeint(
                func=self.blurFunction,
                y0=torch.Tensor(batch_test),
                t=self.blurring_times,
                ...
                if self.final_sharpening == True:
                    if ds_name != "amazon-book":
                        sharpened_out = odeint(
                            func=self.sharpenFunction,
                            y0=self.idl_beta * idl_out[-1] + blurred_out[-1],
                            t=self.sharpening_times,
        ...
        if self.t_point_combination == True:
            if self.sharpening_off == False:
                U_2 = torch.mean(
                    torch.cat([blurred_out[1:, ...], sharpened_out[1:, ...]], axis=0),
                    axis=0,
                )
            else:
...
class BSPM_TORCH(object):
    def __init__(self, adj_mat, config: dict):
        ...
        print("idl time: ", self.idl_times)
        self.blurring_times = (
            torch.linspace(0, blur_T, blur_K + 1).float().to(self.device)
        )
        print("blur time: ", self.blurring_times)
        self.sharpening_times = (
            torch.linspace(0, sharpen_T, sharpen_K + 1).float().to(self.device)
        )
        print("sharpen time: ", self.sharpening_times)
    ...
    def train(self):
        ...
        self.linear_Filter = (
            self.convert_sp_mat_to_sp_tensor(linear_Filter).to_dense().to(self.device)
        )

        if self.config["dataset"] != "amazon-book":
            left_mat = self.d_mat_i @ self.vt.T
            right_mat = self.vt @ self.d_mat_i_inv
            self.left_mat, self.right_mat = torch.FloatTensor(left_mat).to(
                self.device
            ), torch.FloatTensor(right_mat).to(self.device)
        end = time.time()
    ...
    def getUsersRating(self, batch_users, ds_name):
            ...
            if self.sharpening_off == False:
                if self.final_sharpening == True:
                    if ds_name != "amazon-book":
                        sharpened_out = odeint(
                            func=self.sharpenFunction,
                            y0=self.idl_beta * idl_out + blurred_out,
                            t=self.sharpening_times,
                            method=self.sharpen_solver,
                        )
                    elif ds_name == "amazon-book":
        ...
        if self.t_point_combination == True:
            if self.sharpening_off == False:
                U_2 = torch.mean(
                    torch.cat(
                        [blurred_out.unsqueeze(0), sharpened_out[1:, ...]], axis=0
                    ),
                    axis=0,
                )
            else:
