...
def filter_triplets(tp, min_uc=5, min_sc=0):
    ...
    if min_uc > 0:
        ...
        tp = tp[tp["user_id"].isin(usercount.index[(usercount >= min_uc).user_id])]
...
def numerize(tp, profile2id, show2id):
    ...
    sid = tp["item_id"].apply(lambda x: show2id[x])
...
def preprocess(DATA_DIR):
    ...
    # Load Data
    raw_data = pd.read_csv(os.path.join(DATA_DIR, "train.csv"), header=0)
    print("원본 데이터\n", raw_data)

    # Filter Data
    raw_data, user_activity, item_popularity = filter_triplets(
        raw_data, min_uc=3, min_sc=0
    )
    ...
    n_heldout_users = int(n_users * 0.2)
    ...
    pro_dir = os.path.join(DATA_DIR, "../../Hydra_lightning/data/")
