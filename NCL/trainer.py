    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            wandb.log(
                {
                    "epoch": epoch_idx,
                    "train/loss_1": train_loss[0],
                    "train/loss_2": train_loss[1],
                }
            )
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                gh_val = list(
                    filter(lambda x: len(x) >= 2, dict2str(valid_result).split(" "))
                )
                wandb.log({i: float(j) for i, j in zip(gh_val[0::2], gh_val[1::2])})
